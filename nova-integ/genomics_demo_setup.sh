...
setup_placement() {
   ...
   local host_name="jfz1r01h10"
   local cn_uuid=$($openstack resource provider list --name $host_name \
                   -c uuid -f value)
   ...
   echo "   " $device_trait $region_trait $fn_trait
   $openstack resource provider trait set $devrp_uuid --trait $fn_trait \
        --trait $region_trait --trait $device_trait > /dev/null
...
create_af_aas_preprog_flavor() {
    local name=$1
    local device_trait=$2
    local function_id=$3

    local dp_name="${name}-af-aas-dp"
    local flavor_name="${name}-af-aas-flavor"
...
{ "name": "$dp_name",
    "groups": [
        {"resources:CUSTOM_ACCELERATOR_FPGA": "1",
         ...
         "trait:CUSTOM_FUNCTION_ID_$function_id": "required"
        }
    ]
}
EOF
...
setup_device_profiles_and_flavors() {
   ...
   create_af_aas_preprog_flavor "genomics" "$device_trait" "$genomics_fn_id"
}
...
if [[ "$DELETE" -eq 0 ]]; then
   ...
   devrp_uuid=''
   setup_placement $genomics_fn_trait
   setup_db $devrp_uuid
   setup_device_profiles_and_flavors
else
